...
sources:
  - title: "OpenAI: GPT-5 System Card, section 5.1.3.1"
    ...
    note: "The SWE-Bench verbosity note is on PDF page 37. The confidence-interval caveat is on page 23; the changed virology evaluation is on page 25."
---

Somewhere in the [GPT-5 system card](https://cdn.openai.com/gpt-5-system-card.pdf#page=37) a footnote exists that should be a standard.

The card reports a SWE-Bench result, then explains every preparedness evaluation in the document ran at the model's maximum trained-in verbosity (which is higher than the "high" setting available in the API). The 74.9% figure published in the GPT-5 launch blog post ran at the API default. It adds, plainly, that changes in verbosity potentially lead to variation in eval performance.
...
Every benchmark number is a number under a configuration. Verbosity setting? Temperature? Was browsing enabled? What was blocklisted? How many rollouts? Was the reported metric pass@1 or pass@12 across the best set? Which variant of the eval, from which version of the dataset, and on what date? Was the model the deployed version or a helpful-only variant? Was the monitoring stack on?
